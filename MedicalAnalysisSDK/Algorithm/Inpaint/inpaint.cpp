...
MaskedImage_P ExpectationMaximization(Inpaint_P imp, int level, bool isShow)
{
    ...
    if (isShow) printf("EM loop (em=%d,nnf=%d) : ", iterEM, iterNNF);

    // EM Loop
    for (emloop=1;emloop<=iterEM;emloop++) {
        if(isShow) printf(" %d", 1+iterEM-emloop);
...
IplImage* inpaint(Inpaint_P imp, IplImage* input, int ** mask, int radius, bool isShow)
{
    int level, y, x;
    ...
    if (isShow) printf("Build pyramid of images...\n");
    ...
    if(isShow) cvNamedWindow("Progression", CV_WINDOW_AUTOSIZE);
    for (level=maxlevel-1 ; level>0 ; level--) {
        ...
        if (isShow) printf("initialize NNF...");
        if (level==maxlevel-1) {
        ...
        target = ExpectationMaximization(imp, level, isShow);
    ...
    // cvShowImage("target", target->image);
...
void freeInpaintingPyramid(Inpaint_P inp)
{
    ...
    if (inp->pyramid != NULL) {
        ...
        // for ( i=0 ; i<inp->nbEltMaxPyramid; ++i)
